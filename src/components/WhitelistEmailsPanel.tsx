...
export function WhitelistEmailsPanel() {
  ...
  const [selected, setSelected] = useState<WhitelistEmailRow | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (countRes.error) {
        ...
        setRows((dataRes.data as WhitelistEmailRow[]) ?? []);
        setTotalCount(countRes.count ?? 0);
      }
  ...
  function openEdit(row: WhitelistEmailRow) {
    setMode("edit");
    setSelected(row);
    setMutateError(null);
  ...
  async function handleDelete(row: WhitelistEmailRow) {
    if (!row.id) return;
    const confirmed = window.confirm(
      "Delete this whitelist email? This cannot be undone."
    );
    if (!confirmed) return;
  ...
  const filtered = rows.filter(row => {
    ...
    return row.email_address?.toLowerCase().includes(q);
  });
    ...
    <section className="space-y-4">
      ...
      {error && (
        <div className="card border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900">
          <p className="font-medium">Unable to load whitelist emails.</p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-700">
          <span>
            Showing {from}–{to} of {totalCount} whitelist emails
          </span>
          <div className="flex items-center gap-2">
            <button
              type="button"
              ...
              {!loading && filtered.length === 0 && (
                <tr>
                  <td
                    ...
                    colSpan={3}
                  >
                    No whitelist emails on this page
