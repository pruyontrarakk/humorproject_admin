...
export function LlmModelsPanel() {
  ...
  const [selected, setSelected] = useState<LlmModelRow | null>(null);
  ...
  async function load() {
    ...
    if (countRes.error) {
      ...
      setRows((dataRes.data as LlmModelRow[]) ?? []);
      setTotalCount(countRes.count ?? 0);
    }
  ...
  function providerNameFor(model: LlmModelRow): string | null {
    const providerId =
      (model.llm_provider_id as string | null | undefined) ??
      (model.provider_id as string | null | undefined) ??
      null;
  ...
  function openEdit(row: LlmModelRow) {
    setMode("edit");
    setSelected(row);
    setMutateError(null);
  ...
  async function handleDelete(row: LlmModelRow) {
    if (!row.id) return;
    const confirmed = window.confirm(
      "Delete this LLM model? This cannot be undone."
    );
    if (!confirmed) return;
    ...
    <section className="space-y-4">
      ...
      {error && (
        <div className="card border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900">
          <p className="font-medium">Unable to load LLM models.</p>
      ...
      <div className={mode === "idle" ? "card overflow-hidden" : "grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)]"}>
        <div className="card overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-700">
            <span>
              Showing {from}–{to} of {totalCount} models
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
