...
type TermRow = {
  ...
  term_type_id?: number | string | null;
...
export function TermsPanel() {
  ...
  async function load() {
    ...
    if (countRes.error) {
      ...
      setRows((dataRes.data as TermRow[]) ?? []);
      setTotalCount(countRes.count ?? 0);
    }
  ...
  async function handleDelete(row: TermRow) {
    if (!row.id) return;
    ...
    const { error } = await supabase.from("terms").delete().eq("id", row.id);
    if (error) {
      setMutateError(error.message);
    ...
    <section className="space-y-4">
      ...
      {error && (
        <div className="card border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900">
          <p className="font-medium">Unable to load terms.</p>
      ...
      <div
        className={
          mode === "idle"
            ...
            : "grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)]"
        }
      >
        <div className="card overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-700">
            <span>
              Showing {from}–{to} of {totalCount} terms
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
            ...
            <table className="min-w-full border-separate border-spacing-y-1 px-2">
              <thead className="sticky top-0 z-10 bg-white backdrop-blur">
                <tr className="text-[0.7rem] uppercase tracking-[0.16em] text-brand-700">
                  ...
                  <th className="px-3 py-2 text-left">Term type id</th>
                ...
                {!loading &&
                  filtered.map(row => (
                    <tr key={row.id} className="transition hover:bg-brand-50">
                      <td className="max-w-[200px] px-3 py-2 align-top text-slate-800">
                        {row.term ?? (
